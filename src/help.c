...
const char *kg_help_lines[] = {
	...
	"│ M-g      goto line      │ M-x      named command  │ M-.      find definition│",
	"│                         │                         │ M-,      jump back      │",
	"│                         │                         │ C-x p v  vc-dir         │",
	"│                         │                         │ C-x p s  git status     │",
	"│                         │                         │ C-x p d  git diff       │",
	...
	"│ C-a/C-e    line bounds  │ C-x r y  yank rect      │ M-arr    word/para      │",
	"│ C-u        numeric arg  │ C-x r d  delete rect    │ M-S-arr  resize window  │",
	"│ C-v/M-v    page dn/up   │ C-x r c  clear rect     │ C-x +    balance windows│",
	"│ C-w/S-Del  cut          │ C-x r t  string rect    │ C-x 0/1  del wnd/others │",
	"│ M-w/C-Ins  copy         │ MULTIPLE CURSORS        │ C-s/C-r  search fwd/bk  │",
	"│ C-y/S-Ins  paste        │ C-c m c  add cursor     │ M-%      query replace  │",
	"│ DEL        del region   │ C-c m n/a next/all occ  │ C-l      recenter       │",
	"│ M-!/M-|    shell cmd ±  │ C-c m j/k cursor dn/up  │ C-g      cancel/clear   │",
	"│                         │ MACROS                  │ C-h/F1   help           │",
	"│                         │ C-x e/F4 exec macro     │ C-z      suspend        │",
	"│                         │                         │                         │",
	"└─────────────────────────┴─────────────────────────┴─────────────────────────┘",
